    ...
    try {
        ...
        if (q.formula || q.distractors) {
            ...
            if (typeof formulaFn === 'string') {
                formulaFn = new Function('return ' + formulaFn)();
            }
            let distractorsFn = q.distractors;
            if (typeof distractorsFn === 'string') {
                distractorsFn = new Function('return ' + distractorsFn)();
            }
            ...
            for (const [key, val] of Object.entries(result.values)) {
                questionText = questionText.replace(new RegExp(`{${key}}`, 'g'), val);
            }
...
function showInstructions() {
    ...
    if (student) {
        ...
        Promise.all([
            database.ref('exam_activation/' + divKey).once('value'),
            firestoreDb.collection('ActiveAssignments').where('active', '==', true).get()
        ]).then(([activationSnap, homeworkSnap]) => {
            ...
            homeworkSnap.forEach(doc => {
                const data = doc.data();
                // Check legacy 'batch' string OR new 'batches' array
                const batchMatch = (data.batch && data.batch.includes("61st")) ||
                    (Array.isArray(data.batches) && data.batches.some(b => b.includes("61st")));
                if (batchMatch) {
                    hasHomework = true;
            ...
            return database.ref('student_sessions/' + safeName).once('value').then(snapshot => {
                ...
                if (data && data.locked) {
                    ...
                    // Update UI text and buttons based on exam status
                    ...
                    if (examActive) {
                        ...
                        if (examInstructions) examInstructions.style.display = 'block';
                        if (examTitle) examTitle.innerText = "Exam Instructions";
                    } else {
                        if (examBtn) examBtn.style.display = 'none';
                        if (examInstructions) examInstructions.style.display = 'none';
                        if (examTitle) examTitle.innerText = "📚 Homework Dashboard";
                    }
