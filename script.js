...
function getStudentSessionKey(name, batch, division) {
    const cleanName = (name || '').replace(/[.#$[\]]/g, '_');
    const cleanBatch = (batch || '').replace(/[.#$[\]]/g, '_');
    const cleanDiv = (division || '').replace(/[.#$[\]]/g, '_');
    return `${cleanBatch}_${cleanDiv}_${cleanName}`;
}
...
function generateQuestionsByLogin(bank, count, studentName) {
    ...
    finalQuestions = finalQuestions.map(q => {
        return { ...q, options: q.options ? shuffle([...q.options]) : [] };
    });
...
function showInstructions() {
    ...
    // Fetch registered students, activations, and deleted static students from Firebase and merge
    Promise.all([
        database.ref('registered_students').once('value'),
        database.ref('exam_activation').once('value'),
        database.ref('deleted_static_students').once('value')
    ]).then(([studentsSnap, activationsSnap, deletedSnap]) => {
        const deletedStaticTcs = deletedSnap.val() || {};
        const filteredStaticStudents = students.filter(s => {
            const key = `${s.batch}_${s.division}_${s.pass}`;
            return !deletedStaticTcs[key];
        });

        let allStudents = [...filteredStaticStudents];
        ...
        if (student) {
            const sessionKey = getStudentSessionKey(student.name, student.batch || '', student.division);
            const divKey = (student.batch || '') + '_' + student.division;

            // ── Step 2: Check session lock ──
            database.ref('student_sessions/' + sessionKey).once('value').then(snapshot => {
                ...
                if (data && data.locked) {
                    ...
                    database.ref('student_sessions/' + sessionKey).set({
                        locked: true,
                        timestamp: Date.now()
                    });
                    ...
                    function updatePortalUI() {
                        ...
                        if (hasExam && assignedExamData && assignedExamData.questions) {
                            // Admin has assigned an exam — use Firestore questions
                            currentQuestions = assignedExamData.questions;
                        } else {
                            // Fallback: no assigned exam, use local question bank
                        ...
                        if (examTitle) {
                            if (hasExam && assignedExamData) {
                                ...
                                localStorage.setItem('currentExamTitle', activeTitle);
                                localStorage.setItem('currentExamFirestoreId', assignedExamData.id || '');
                            } else {
                                examTitle.innerText = "Exam Instructions";
                        ...
                        if (examInstructions) {
                            if (hasExam && assignedExamData) {
                                ...
                                // Default instructions using local question bank
                                examInstructions.innerHTML = `
                                    <li>✅ <b>Duration:</b> 90 Minutes</li>
                                    <li>✅ <b>Security:</b> Fullscreen mode is mandatory.</li>
                                    <li>⚠️ <b>Warning:</b> Leaving the tab twice will auto-submit.</li>`;
                            }
                    ...
                    // Listen for Active Exam ID (RTDB & Firestore fallback)
                    let examUnsubscribe = null;
                    const batchOnlyKey = (student.batch || '').split(' ')[0].replace(/[\s-]/g, '_');

                    function handleExamDocSnapshot(doc) {
                        if (doc && doc.exists && doc.data().active !== false && Array.isArray(doc.data().questions) && doc.data().questions.length > 0) {
                            hasExam = true;
                    ...
                    function fallbackDirectFirestoreExamSearch() {
                        if (typeof firestoreDb === 'undefined') return;
                        firestoreDb.collection('ActiveExams').where('active', '==', true).get().then(snap => {
                            let foundExam = null;
                            const stBatch = (student.batch || '').toLowerCase();
                            const stDiv = (student.division || '').toLowerCase();

                            snap.forEach(doc => {
                                const data = doc.data();
                                const examBatch = (data.batch || '').toLowerCase();
                                if (examBatch.includes(stBatch) || (stDiv && examBatch.includes(stDiv))) {
                                    if (Array.isArray(data.questions) && data.questions.length > 0) {
                                        foundExam = { id: doc.id, ...data };
                                    }
                                }
                            });
                            ...
                            console.error("Direct Firestore exam query fallback error:", err);
                            hasExam = false;
                            assignedExamData = null;
                            updatePortalUI();
                        });
                    ...
                    function bindExamDoc(newExamId) {
                        if (examUnsubscribe) {
                            examUnsubscribe();
                        ...
                        if (newExamId) {
                            ...
                            examUnsubscribe = firestoreDb.collection('ActiveExams').doc(activeExamId)
                                .onSnapshot(doc => {
                                    if (!handleExamDocSnapshot(doc)) {
                                        fallbackDirectFirestoreExamSearch();
                                    }
                                }, err => {
                            ...
                            // Check batch-only RTDB key before falling back
                            database.ref('active_exams/' + batchOnlyKey).once('value').then(bSnap => {
                                const bData = bSnap.val();
                                if (bData && bData.firestoreId) {
                                    bindExamDoc(bData.firestoreId);
                                } else {
                                    fallbackDirectFirestoreExamSearch();
                                }
                            }).catch(() => fallbackDirectFirestoreExamSearch());
                        }
                    }
...
function nextQuestion() {
    if (currentQuestionIndex < currentQuestions.length - 1) {
        ...
        const confirmed = confirm("Submit Exam?");
...
// ===============================
// 📊 FIX: PREVENTING "UNDEFINED" IN REPORT & SAFE SUBMISSION
// ===============================
function calculateFinalScore() {
    ...
    try {
        ...
        const reportDetails = safeQuestions.map((q, i) => {
            if (!q) return { question: "N/A", userSelection: "Not Answered", correctAnswer: "N/A", status: false };
            const correctAns = q.answer || q.correct || q.correctAnswer || "";
            const isCorrect = userAnswers && userAnswers[i] === correctAns;
            if (isCorrect) score++;
            return {
                question: q.q || q.question || "Question",
                userSelection: (userAnswers && userAnswers[i]) ? userAnswers[i] : "Not Answered",
        ...
        const totalQ = safeQuestions.length > 0 ? safeQuestions.length : 1;
        ...
        if (typeof database !== 'undefined') {
            database.ref('exam_results').push({
                ...
                violations: typeof violationCount !== 'undefined' ? violationCount : 0,
        ...
        const rSection = document.getElementById('result-section');
        ...
        if (finalTotalEl) finalTotalEl.innerText = safeQuestions.length;
        if (resultPercentEl) resultPercentEl.innerText = `Percentage: ${finalPercent}%`;
...
let isShowingConfirmDialog = false;

function handleViolation() {
    if (isShowingConfirmDialog) return; // Ignore if browser confirm dialog is open
    const now = Date.now();
...
function checkAndStartUserListener() {
    ...
    const currentTC = localStorage.getItem("currentTC");
    if (currentStudent && typeof database !== 'undefined') {
        // Sanitize name (spaces + special chars) to match what admin writes
        const safeName = currentStudent.replace(/[.#$[\]\s]/g, '_');
        // Listen on safeName format (primary — matches admin reset)
        database.ref('account_resets/users/' + safeName).on('value', (snapshot) => {
            const data = snapshot.val();
            if (data && data.timestamp) handleResetSignal(data.timestamp, "User Reset (name)");
        });
        // Also listen on safeUid (name_TC) format as secondary
        if (currentTC) {
            const safeUid = safeName + '_' + currentTC;
            database.ref('account_resets/users/' + safeUid).on('value', (snapshot) => {
                const data = snapshot.val();
                if (data && data.timestamp) handleResetSignal(data.timestamp, "User Reset (uid)");
            });
        }
...
async function submitHomework() {
    ...
    // Build question-wise detail breakdown removed to prevent students from seeing/sharing correct answers.
    document.getElementById('hw-result-detail').innerHTML = '';
}
